...
import { TsConfig, ExtraCommandLineOptions } from './types';
...
export const loadTsConfig = (
  ...
  return { baseUrl, paths, files };
};

export default (tsconfigPath: string): Analysis => {
  const options: ExtraCommandLineOptions = {};
  const tsConfig = loadTsConfig(tsconfigPath);

  return analyze(parseFiles(tsConfig, options), options);
};
